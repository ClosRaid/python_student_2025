...
def main():
    ...
    while True:
        ...
        response = input('> ') #задание 3 -> если смотреть 19 строчку, то ничего, если эту то будет ошибка что не
                                #найдена переменна
        if not response.isdecimal():
            print('Please enter a number.')
        else:
            numberOfHeadlines = int(response) #задание 2 -> нельзя в рендж вставить строчку
            break # Выходим из цикла, когда будет введено допустимое число.

    for i in range(numberOfHeadlines): #1 задание -> не найдётся вот эта строчка и выйдет ошибка
